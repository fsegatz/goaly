...
describe('UIController', () => {
    ...
    test('goalyLogo touchstart/touchend should navigate to dashboard on mobile', () => {
        // Set up developer mode handlers (which now include navigation logic)
        const DeveloperModeService = require('../src/domain/services/developer-mode-service').default;
        const DEVELOPER_MODE_PRESS_DURATION_MS = require('../src/domain/utils/constants').DEVELOPER_MODE_PRESS_DURATION_MS;
        
        mockApp.developerModeService = new DeveloperModeService();
        mockApp.setupDeveloperMode = function() {
            ...
            const startPress = (e) => {
                ...
                pressTimer = setTimeout(() => {
                    mockApp.developerModeService.enable();
                    pressTimer = null;
        ...
        // Simulate quick touch (mobile tap) - create events manually since TouchEvent may not be available
        const touchStartEvent = document.createEvent('Event');
        touchStartEvent.initEvent('touchstart', true, true);
        goalyLogo.dispatchEvent(touchStartEvent);
        
        // Wait a bit (but less than 300ms threshold)
        jest.advanceTimersByTime(100);
        
        // Simulate touchend (quick tap)
        const touchEndEvent = document.createEvent('Event');
        touchEndEvent.initEvent('touchend', true, true);
        goalyLogo.dispatchEvent(touchEndEvent);
    ...
    test('goalyLogo touchmove should prevent navigation', () => {
        // Set up developer mode handlers (which now include navigation logic)
        const DeveloperModeService = require('../src/domain/services/developer-mode-service').default;
        const DEVELOPER_MODE_PRESS_DURATION_MS = require('../src/domain/utils/constants').DEVELOPER_MODE_PRESS_DURATION_MS;
        ...
        // Simulate touch with movement - create events manually
        const touchStartEvent = document.createEvent('Event');
        touchStartEvent.initEvent('touchstart', true, true);
        goalyLogo.dispatchEvent(touchStartEvent);
        
        // Simulate touchmove (user is scrolling/dragging)
        const touchMoveEvent = document.createEvent('Event');
        touchMoveEvent.initEvent('touchmove', true, true);
        goalyLogo.dispatchEvent(touchMoveEvent);
        ...
        // Should NOT switch to dashboard (because touch moved)
        expect(allGoalsView.classList.contains('active')).toBe(true);
        expect(dashboardView.classList.contains('active')).toBe(false);
    });
});
